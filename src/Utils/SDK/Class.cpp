...
ServerClass::ServerClass(const char *pNetworkName, SendTable *pTable) {
	...
	m_pTable = pTable;
	m_InstanceBaselineIndex = 65535;
	// g_pServerClassHead is sorted alphabetically, so find the correct place to insert
	auto g_pServerClassHead = server->GetAllServerClasses();
	if (!g_pServerClassHead) {
		...
		if (_stricmp(p1->m_pNetworkName, pNetworkName) > 0) {
		...
		while (p1) {
			if (p2 == NULL || _stricmp(p2->m_pNetworkName, pNetworkName) > 0) {
				m_pNext = p2;
				p1->m_pNext = this;
				break;
			}
